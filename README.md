...
- **OpenCode decides the model** — summarization runs on the native `compaction` agent, so the model configured in `agent.compaction.model` (e.g. a cheap one) pays the summary bill, never the working model.
...
- **Terminal notice** — one ignored line per trigger (`▣ auto-compact: 51.2% ≥ 50% — compacting`). UI-only: never sent to the model (`ignored` part + `noReply` prompt).
